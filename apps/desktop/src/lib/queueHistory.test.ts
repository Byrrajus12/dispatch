...
describe('groupFailedAttempts', () => {
  test('live failures fail, merged entries are skipped, nothing is invented', () => {
  ...
  // The original bug: "run was already reviewed" failures headlining as if they
  // still needed someone. A reviewed run's failed attempt is stale history, not news.
  ...
  // History is newest-first: an older failed attempt for a run whose newer attempt
  // merged (or failed again) is superseded — only the latest attempt is the run's story.
  ...
  // Clicking a failed row's Retry puts the run back in the live queue. From that
  // moment the pending attempt is the run's story — the old failure demotes to stale
  // instead of headlining next to its own retry.
...
describe('latestFailedAttemptByRunId', () => {
  test('keeps a run only when its newest attempt failed, with that attempt', () => {
    const latest = latestFailedAttemptByRunId([
      entry('a', 'failed', { reason: 'second' }),
      entry('a', 'merged'),
      entry('b', 'merged'),
      entry('b', 'failed'),
      entry('c', 'failed', { reason: 'only' }),
    ]);
    expect([...latest.keys()]).toEqual(['a', 'c']);
    expect(latest.get('a')?.reason).toBe('second');
    expect(latest.get('c')?.reason).toBe('only');
  });

  test('empty history flags nothing', () => {
    expect(latestFailedAttemptByRunId([]).size).toBe(0);
  });
});
